...
export const MetricCards = ({ totals = {}, summary = {} }) => (
  <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <Card key={metric.key} className="p-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-2xl font-bold text-slate-950 dark:text-slate-50">{compactNumber(totals[metric.key] || 0)}</p>
            </div>
    ...
    <Card className="border-emerald-300 bg-emerald-400 p-4 text-emerald-950 dark:border-emerald-400 dark:bg-emerald-400">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-wider">Unchecked Items</p>
          <p className="mt-2 text-3xl font-black">{compactNumber(summary.unchecked || 0)}</p>
        </div>
        <FiEdit3 className="h-5 w-5" />
      </div>
      <p className="mt-2 text-sm font-semibold opacity-80">Requires review</p>
    </Card>
  </div>
);
